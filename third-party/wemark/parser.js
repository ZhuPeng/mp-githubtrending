...
function parse(md, options){
  ...
  var parseHtml = function(html, ret) {
    var list = [{ type: 'text', reg: /<a.*?>(.*?)<\/a>/g }, { type: 'image', reg: /<img.*?src\s*=\s*['"]*([^\s^'^"]+).*?(?:\/\s*|<\/img)?>/g }, { type: 'text', reg: /<h2.*?>(.*?)<\/h2>/g }, { type: 'text', reg: /<span.*?>(.*?)<\/span>/g }, { type: 'text', reg: /<h1.*?>(.*?)<\/h1>/g }, { type: 'text', reg: /<p.*?>(.*?)<\/p>/g }, { type: 'text', reg: /<p.*?>(.*)/g }, { type: 'text', reg: /<div.*?>(.*)<\/div>/g }, { type: 'text', reg: /<h3.*?>(.*?)<\/h3>/g }]
    var match;
    list.map(function(p) {
      var tmpHtml = html
    ...
    if (html) {
      console.log('remain unparsed html:', html)
